export default {
  ...
  settings: {
    ...
    barcodeScan: {
      title: '扫描添加条形码查询',
      enableLookup: '在线查询扫描到的条形码',
      enableLookupDesc: '当扫描到的条形码尚未存在于您的库存中时，查询社区维护的 Open Filament Database 和 SpoolmanDB-Community。关闭后仅匹配您以前扫描过的条形码——扫描功能本身始终可用。',
      refreshNow: '立即刷新数据库',
      databaseRefreshed: 'Open Filament Database 已刷新——现有 {{count}} 个条形码',
      databaseRefreshFailed: '刷新 Open Filament Database 失败',
    },
    ...
    colorCatalog: {
      ...
      syncSpoolmanDbCommunityTooltip: '从 SpoolmanDB-Community 同步（430+ 个品牌，可能需要一分钟）',
      loadFailed: '加载颜色目录失败',
  ...
  inventory: {
    ...
    barcodeScan: {
      ...
      tabManual: '手动输入',
      scanHint: '将条形码对准取景框',
      cameraNeedsHttps: '摄像头不可用——需要 HTTPS（或 localhost）。请改用标签照片或手动输入。',
      cameraError: '摄像头不可用（{{error}}）。如果 Bambuddy 未通过 HTTPS 提供服务，这是正常现象——请改用标签照片或手动输入。',
      photoHint: '拍摄或选择料盘标签的照片——即使不支持摄像头扫描也可使用。',
      takePhoto: '拍照',
      choosePhoto: '选择照片',
      manualHint: '输入料盘包装上印刷的条形码或制造商 SKU/货号',
      manualPlaceholder: '例如 6938936716785',
      manualInvalid: '请输入有效的 UPC-A、EAN-8 或 EAN-13 条形码，或制造商 SKU/货号（至少 3 个字符）',
      lookUp: '查询',
      lookingUp: '正在查询条形码…',
      sourceInventory: '您的库存',
      sourceOfd: 'Open Filament Database',
      sourceSpoolmanDbCommunity: 'SpoolmanDB-Community',
      matchedToast: '已从{{source}}匹配到结果',
      noMatchToast: '未找到匹配项——请手动填写详细信息',
      lookupFailed: '条形码查询失败',
      ocrReading: '正在读取标签…',
      ocrProgress: '正在读取标签… {{progress}}',
      ocrNoText: '未识别到文字——请尝试更清晰、更近的照片。',
      parsingLabel: '正在解析标签…',
      guessedToast: '已根据标签猜测详细信息——请核对',
      ocrFailed: '读取标签失败',
    },
    addSpool: '添加耗材',
    editSpool: '编辑耗材',
    ...
    costPerKg: '每公斤成本',
    barcode: '条形码',
    barcodePlaceholder: '例如 6938936716785',
    barcodeHelp: '由"扫描条形码"自动填写。也可手动输入，以便日后再次扫描相同条形码时立即匹配到此料盘。',
    linkedCodesLabel: '同时匹配：{{codes}}',
    linkedCodesRefill: '补充装',
